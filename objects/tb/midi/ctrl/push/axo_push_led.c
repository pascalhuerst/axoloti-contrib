...
void PushInitLed(Push& p) {
	...
	const char* welcome="HUBABUBA";
	PushSetRow(p,0,welcome,strlen(welcome)); // TEST

	PushDbgLog("Setting text...");

	for (int r=0; r<4; r++) {
		for (int c=0; c<8; c++) {
			char txt[10];
			chsnprintf(txt, 10, "(%i|%i)", c, r);
			//PushSetRow(p, i, txt, strlen(txt));
			PushSetCell(p, r, c, txt, strlen(txt));
		}
	}

//	PushUnipolarBar(p, 0, 0, 77);
//	PushBipolarBar(p, 0, 0, -23);
